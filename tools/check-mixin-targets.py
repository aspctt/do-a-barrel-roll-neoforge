...
- every @Shadow method exists on the target class or something it inherits from, with the same signature
...
The last form checks against another NeoForge build a target accepts, once Gradle has set up its game jar by
writing out its classpath: ./gradlew :26.1:writeCompileClasspath -Pneo_version=26.1.0.19-beta
-Pminecraft_version=26.1. Compiling against that build instead would replace the classes the linkage check
reads with ones that were never published.
...
MIXIN_PACKAGE = "nl/enjarai/doabarrelroll/mixin"
MIXIN_CONFIG = "do_a_barrel_roll.mixins.json"
...
class Classpath:
	...
	def has_method(self, cls, name, desc, seen=None):
		"""Whether cls or any supertype declares name with desc; None if cls is not on the classpath."""
		# No depth cap: LocalPlayer.turn is declared on Entity, five classes up since 1.21.9 added Avatar.
		...
		if (name, desc) in methods:
			return True
		return any(self.has_method(sup, name, desc, seen) for sup in self.supertypes(cls))
...
def check_version(version, neo=None):
	...
	# Mixins sit in subpackages, and the config names each one relative to the mixin package.
	mixins = sorted(os.path.relpath(os.path.join(d, f), root)[:-5].replace(os.sep, ".")
					for d, _, files in os.walk(root) for f in files if f.endswith(".java"))

	problems, notes, checked = [], [], 0
	for mixin in mixins:
		if active is not None and mixin not in active:
			continue
		# Two mixins can share a class name in different packages, so reports carry the package.
		name = mixin + ".java"
		src = Source(os.path.join(root, *mixin.split(".")) + ".java")
		text = src.text
		m = re.search(r'@Mixin\((?:value\s*=\s*)?([A-Za-z0-9_.]+)\.class\b', text)
		if not m:
			continue
		...
		# Shadowed methods may be inherited, the same way a call through them resolves.
		for sh in re.finditer(r'@Shadow\s+((?:@\w+\s+)*)((?:(?:private|public|protected|static|final|abstract)\s+)*)([\w<>\[\].]+)\s+([\w$]+)\s*\(([^)]*)\)\s*;', text):
			checked += 1
			params = [p.rsplit(None, 1)[0] for p in split_top_level(sh.group(5)) if p]
			desc = "(" + "".join(src.descriptor(p) for p in params) + ")" + src.descriptor(sh.group(3))
			if not cp.has_method(target, sh.group(4), desc):
				problems.append("%s: %s declares no method %s%s" % (name, target, sh.group(4), desc))
